...
reprod = False
if reprod:
    d = np.load('reproduce1.npz'); W_rec = d['W_rec']; W_ff = d['W_ff']; sign = np.diag(np.ones(W_rec.shape[0]))
    W_fb = d['W_fb']; US = d['US'][:n_pat]; CS = d['CS'][:n_pat]
else:
    W_rec = None; W_ff = None; W_fb = None; US = None; CS = None; sign = None
